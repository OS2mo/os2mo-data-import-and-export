...
imports_sd_changed_at(){
    ...
    python3 integrations/SD_Lon/sd_changed_at.py
}
...
exports_viborg_eksterne(){
    ...
    python3 exporters/viborg_eksterne/viborg_eksterne.py --lora|| exit 1
    (
...
reports_sd_db_overview(){
    ...
    outfile=$(mktemp)
    python3 integrations/SD_Lon/db_overview.py > ${outfile}
    local STATUS=$?
    head -2 ${outfile}
    echo "..."
    tail -3 ${outfile}
