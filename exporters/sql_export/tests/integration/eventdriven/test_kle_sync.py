...
async def test_kle_lifecycle(
    ...
    def expected(uuid: str, number_uuid: str, number_titel: str) -> dict[str, Any]:
        return {
            "uuid": uuid,
    ...
    # Delete (KLE has no _delete mutation; terminate removes it)
    await terminate("kle", uuid)
    await assert_absent(session, KLE)
