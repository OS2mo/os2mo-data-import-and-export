...
from .autogenerated_graphql_client.query_engagements import (
    QueryEngagementsEngagementsObjects,
)
...
class GQLLoraCache:
    def __init__(
        ...
        settings=None,
        graphql_session: GraphQLClient | None = None,
        codegen_client: CodegenGraphQLClient | None = None,
    ):
        logger.info(
        ...
        self.gql_client_session = graphql_session or self._setup_gql_client()
        self.codegen_client = codegen_client or self._setup_codegen_gql_client()
    ...
    def _setup_codegen_gql_client(self) -> CodegenGraphQLClient:
        http_client = AuthenticatedAsyncHTTPXClient(
            client_id=self.settings.client_id,
            client_secret=self.settings.client_secret,
            token_endpoint=f"{self.settings.auth_server}/realms/{self.settings.auth_realm}/protocol/openid-connect/token",
            timeout=300,
        )
        client = CodegenGraphQLClient(
    ...
    async def _execute_codegen_query(
        self, query, uuid: UUID | None = None
    ) -> AsyncGenerator[QueryEngagementsEngagementsObjects, Any]:
        """Executes a query using the autogenerated client.

        Input a query like "self.codegen_client.query_engagements" and an optional uuid.

        Returns an async generator which yields one result at a time.
        If an uuid is given only that object is returned. If not we will use
        pagination to return all objects matching the query.

        """
        from_date = datetime.datetime.now() if not self.full_history else None
        to_date = (
            datetime.datetime.now() + datetime.timedelta(seconds=5)
            if not self.full_history
            else None
        )
        limit = None if uuid else 100
        cursor = None
        while True:
    ...
    async def _fetch_engagements_codegen(
        ...
        result = {}
        # TODO: Maybe add timezones?

        async for obj in self._execute_codegen_query(
            self.codegen_client.query_engagements,
            uuid=uuid,
        ):
            result[str(obj.uuid)] = [
                {
                    "engagement_type": str(o.engagement_type.uuid),
                    "extensions": {
                        "udvidelse_1": o.extension_1,
                        "udvidelse_2": o.extension_2,
                        "udvidelse_3": o.extension_3,
                        "udvidelse_4": o.extension_4,
                        "udvidelse_5": o.extension_5,
                        "udvidelse_6": o.extension_6,
                        "udvidelse_7": o.extension_7,
                        "udvidelse_8": o.extension_8,
                        "udvidelse_9": o.extension_9,
                        "udvidelse_10": o.extension_10,
                    },
                    "fraction": o.fraction,
                    "from_date": str(o.validity.from_.date()),
                    "to_date": (
                        str(o.validity.to.date()) if o.validity.to else "9999-12-31"
                    ),
                    "job_function": str(o.job_function.uuid),
                    "primary_boolean": o.is_primary,
                    "primary_type": str(o.primary.uuid) if o.primary else None,
                    # There can be several validities for an org_unit or person but only one uuid
                    "unit": one({str(org.uuid) for org in o.org_unit}),
                    # Vacant engagements exists, so there is a possibility of no persons being attached
                    "user": only({str(person.uuid) for person in o.person}),
                    "user_key": o.user_key,
                    "uuid": str(o.uuid),
                }
                for o in obj.validities
            ]
        return result
