...
class GQLLoraCache:
    ...
    async def _fetch_facets(self, uuid: UUID | None = None) -> dict:
        ...
        async for obj in self._execute_query(
            query=query, variable_values={"filter": {"uuids": uuid}}, page=bool(uuid)
        ):
            if obj is None:
                return {}
    ...
    async def _fetch_engagements(self, uuid: UUID | None = None) -> dict:
        ...
        res: dict = {}
        async for obj in self._execute_query(
            query=query, uuid=uuid, variable_values=variables, page=bool(uuid)
        ):
            if obj is None:
                return {}
            if not self.full_history:
        ...
        query = """
            query (
                ...
                page: related_units(
                    ...
                    objects {
                        uuid
                        current @include(if: $current) {
                            ...RelatedUnitFragment
                        }
                        obj: validities @skip(if: $current) {
                            ...RelatedUnitFragment
                        }
                    }
                    page_info {
            ...
            fragment RelatedUnitFragment on RelatedUnit {
                uuid
                org_unit_uuids
                validity {
                    from
