...
class SqlExport:
    ...
    def _gen_sql_facet(self, uuid, facet_info):
        return Facet(
            uuid=uuid,
    ...
    def _add_facets(self):
        ...
        for chunk in ichunked(facets, self.chunk_size):
            for uuid, facet_info in chunk:
                sql_facet = self._gen_sql_facet(uuid, facet_info)
                self.session.add(sql_facet)
            self.session.commit()

    def _gen_sql_class(self, uuid, klasse_info):
        return Klasse(
            uuid=uuid,
    ...
    def _add_classes(self):
        ...
        for chunk in ichunked(classes, self.chunk_size):
            for uuid, klasse_info in chunk:
                sql_class = self._gen_sql_class(uuid, klasse_info)
                self.session.add(sql_class)
            self.session.commit()

    def _gen_sql_user(self, uuid, user_info):
        return Bruger(
            uuid=uuid,
        ...
        for chunk in ichunked(users, self.chunk_size):
            for uuid, user_effects in chunk:
                for user_info in user_effects:
                    sql_user = self._gen_sql_user(uuid, user_info)
                    self.session.add(sql_user)
            self.session.commit()

    def _gen_sql_unit(self, uuid, unit_info):
        ...
        for chunk in ichunked(units, self.chunk_size):
            for uuid, unit_validities in chunk:
                for unit_info in unit_validities:
                    sql_unit = self._gen_sql_unit(uuid, unit_info)
                    self.session.add(sql_unit)
            self.session.commit()

    def _gen_sql_engagement(self, uuid, engagement_info):
        ...
        for chunk in ichunked(engagements, self.chunk_size):
            for uuid, engagement_validity in chunk:
                for engagement_info in engagement_validity:
                    sql_engagement = self._gen_sql_engagement(uuid, engagement_info)
                    self.session.add(sql_engagement)
            self.session.commit()

    def _gen_sql_addresse(self, uuid, address_info):
        ...
        for chunk in ichunked(addresses, self.chunk_size):
            for uuid, address_validities in chunk:
                for address_info in address_validities:
                    sql_address = self._gen_sql_addresse(uuid, address_info)
                    self.session.add(sql_address)
            self.session.commit()

    def _gen_sql_dar_addresse(self, uuid, dar_addresse_info):
        return DARAdresse(
            uuid=uuid,
            **{
                key: value
                for key, value in dar_addresse_info.items()
    ...
    def _add_dar_addresses(self):
        ...
        for chunk in ichunked(dar, self.chunk_size):
            ...
            for uuid, address_info in chunk:
                sql_address = self._gen_sql_dar_addresse(uuid, address_info)
                self.session.add(sql_address)
            self.session.commit()

    def _gen_sql_association(self, uuid, association_info):
        (
        ...
        for chunk in ichunked(associations, self.chunk_size):
            ...
            for uuid, association_validity in chunk:
                for association_info in association_validity:
                    sql_association = self._gen_sql_association(uuid, association_info)
                    self.session.add(sql_association)
            self.session.commit()

    def _gen_sql_role(self, uuid, role_info):
        return Rolle(
            uuid=uuid,
        ...
        for chunk in ichunked(roles, self.chunk_size):
            ...
            for uuid, role_validity in chunk:
                for role_info in role_validity:
                    sql_role = self._gen_sql_role(uuid, role_info)
                    self.session.add(sql_role)
            self.session.commit()

    def _gen_sql_leave(self, uuid, leave_info):
        ...
        for chunk in ichunked(leaves, self.chunk_size):
            ...
            for uuid, leave_validity in chunk:
                for leave_info in leave_validity:
                    sql_leave = self._gen_sql_leave(uuid, leave_info)
                    self.session.add(sql_leave)
            self.session.commit()

    def _gen_sql_it_system(self, uuid, it_system_info):
        return ItSystem(uuid=uuid, navn=it_system_info["name"])
    ...
    def _add_it_systems(self):
        ...
        for chunk in ichunked(itsystems, self.chunk_size):
            for uuid, it_system_info in chunk:
                sql_itsystem = self._gen_sql_it_system(uuid, it_system_info)
                self.session.add(sql_itsystem)
            self.session.commit()

    def _gen_sql_it_user(self, uuid, it_user_info):
        return ItForbindelse(
            uuid=uuid,
            it_system_uuid=it_user_info["itsystem"],
            bruger_uuid=it_user_info["user"],
            enhed_uuid=it_user_info["unit"],
            brugernavn=it_user_info["username"],
            startdato=it_user_info["from_date"],
            slutdato=it_user_info["to_date"],
            primær_boolean=it_user_info.get("primary_boolean"),
        )
    ...
    def _add_it_users(self):
        ...
        for chunk in ichunked(it_connections, self.chunk_size):
            ...
            for uuid, it_connection_validity in chunk:
                for it_user_info in it_connection_validity:
                    sql_it_connection = self._gen_sql_it_user(uuid, it_user_info)
                    self.session.add(sql_it_connection)
            self.session.commit()

    def _gen_sql_kle(self, uuid, kle_info):
        return KLE(
            uuid=uuid,
        ...
        for chunk in ichunked(kles, self.chunk_size):
            ...
            for uuid, kle_validity in chunk:
                for kle_info in kle_validity:
                    sql_kle = self._gen_sql_kle(uuid, kle_info)
    ...
    def _gen_sql_related(self, uuid, related_info):
        return Enhedssammenkobling(
            uuid=uuid,
        ...
        for chunk in ichunked(relateds, self.chunk_size):
            ...
            for uuid, related_validity in chunk:
                for related_info in related_validity:
                    sql_related = self._gen_sql_related(uuid, related_info)
                    self.session.add(sql_related)
            self.session.commit()

    def _gen_sql_manager(self, uuid, manager_info) -> tuple[Leder, list[LederAnsvar]]:
        manager = Leder(
            uuid=uuid,
        ...
        for responsibility in manager_info["manager_responsibility"]:
            manager_responsibilities.append(
                LederAnsvar(
                    leder_uuid=uuid,
        ...
        return manager, manager_responsibilities
    ...
    def _add_managers(self):
        ...
        for chunk in ichunked(managers, self.chunk_size):
            ...
            for uuid, manager_validity in chunk:
                for manager_info in manager_validity:
                    sql_manager, manager_responsibilities = self._gen_sql_manager(
                        uuid, manager_info
                    )
                    self.session.add(sql_manager)
                    for sql_responsibility in manager_responsibilities:
