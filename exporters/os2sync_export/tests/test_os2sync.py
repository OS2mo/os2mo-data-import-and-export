...
def test_os2sync_upsert_orgunit_no_changes(get_settings_mock):
...
def test_os2sync_upsert_orgunit_changes(get_settings_mock):
    ...
    with patch("os2sync_export.os2sync.os2sync_get_org_unit", return_value=o.copy()):
        with patch("os2sync_export.os2sync.os2sync_post") as post_mock:
            ...
            upsert_orgunit(org_unit, "os2sync_api_url")
            post_mock.assert_called_once_with("{BASE}/orgUnit/", json=org_unit.json())


@patch("os2sync_export.config.get_os2sync_settings", return_value=dummy_settings)
def test_os2sync_upsert_orgunit_keep_fk_fields(get_settings_mock):
...
@patch("os2sync_export.config.get_os2sync_settings", return_value=dummy_settings)
def test_os2sync_upsert_orgunit_changes_w_fixed_fields(get_settings_mock):
